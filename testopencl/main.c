//test OpenCL code based on https://developer.apple.com/library/content/documentation/Performance/Conceptual/OpenCL_MacProgGuide/ExampleHelloWorld/Example_HelloWorld.html#//apple_ref/doc/uid/TP40008312-CH112-SW2
...
static void seq_benchfunc(float* input, float* output) {
    int i;
    for (i = 0; i < NUM_VALUES; i++) {
...
int main (int argc, const char * argv[]) {
    ...
    clGetDeviceInfo(gpu, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(cl_ulong), &dev_mem_size, NULL);
    fprintf(stdout, "Device memory: %7.2f MB\n", (float)dev_mem_size / MEGA);
    
    cl_ulong dev_alloc_size;
    clGetDeviceInfo(gpu,  CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(cl_ulong), &dev_alloc_size, NULL);
    fprintf(stdout, "Maximum allocatable memory block: %7.2f MB\n", (float)dev_alloc_size / MEGA);
    
   
    
    bool inplace = true;
    
    cl_ulong invecsize = NUM_VALUES * sizeof(cl_float);
    
    fprintf(stdout, "Input vector size: %7.2f MB\n", (float) invecsize/ MEGA);
    
    //why half the size? inplace or not the limit is the same
    if (invecsize > dev_mem_size /2 ) {
        fprintf(stdout, "Input vector is too big, cannot perform calculation\n");
        exit(1);
    }
    ...
    //Start measuring time before moving data
    clock_t begin = clock();
    ...
    void* mem_in  = inplace? gcl_malloc(sizeof(cl_float) * NUM_VALUES, test_in,
    ...
    dispatch_sync(queue, ^{
        ...
        size_t wgs;
        gcl_get_kernel_block_workgroup_info(bench_kernel,
        ...
        bench_kernel(&range,(cl_float*)mem_in, (cl_float*)mem_out);
    ...
    clock_t end = clock();
    double opencl_time_spent = (double)(end - begin) / CLOCKS_PER_SEC;
    
    // Don't forget to free up the CL device's memory when you're done. // 10
    gcl_free(mem_in);
    if (!inplace) gcl_free(mem_out);
    
    
    //cpu computing (without opencl)
    
    float* seq_out = (float*)malloc(sizeof(float) * NUM_VALUES);
    
    
    begin = clock();
    
    seq_benchfunc(test_in, seq_out);
    
    end = clock();
    
    //test_in is no more used, we can free up some memory
    free(test_in);
    ...
    if ( validate(test_out, seq_out, 0.0002)) {
        fprintf(stdout, "All values were properly computed.\n");
    }
    
    fprintf(stdout, "sequential time spent: %f\n", seq_time_spent);
    fprintf(stdout, "OpenCL time spent: %f\n", opencl_time_spent);
    
    
    
    // Cleanup other system mem
